...
void TestTask::setUp(){
  ...
  std::string valid_task_json = "{'id': 1, 'title': 'first title', 'description': 'some description'}";
  valid = new Json::Value();
  std::string errors;
  reader->parse(valid_task_json.c_str(), valid_task_json.c_str() + valid_task_json.size(), valid, &errors);
  // create Task objects
  t1 = new Task(valid);
  ...
  reader->parse(no_id_json_string.c_str(), no_id_json_string.c_str() + no_id_json_string.size(), noId, &errors);

  // no title json
  noTitle = new Json::Value();
  std::string no_title_json = "{'id': 10}";
  reader->parse(no_title_json.c_str(), no_title_json.c_str() + no_title_json.size(), noTitle, &errors);
}
